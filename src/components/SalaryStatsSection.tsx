...
export function SalaryStatsSection() {
  ...
  return (
    <section className="w-full py-24 relative bg-surface-secondary/30 border-y border-border">
      <div className="container mx-auto px-4 relative z-[2]">
        <FadeIn>
          <div className="flex flex-col md:flex-row justify-between items-end mb-12">
            <div>
              <h2 className="text-accent font-mono text-sm font-semibold tracking-wide uppercase mb-2">Market Data</h2>
              <h3 className="text-3xl md:text-4xl font-bold text-foreground">Why Learn Full Stack?</h3>
            </div>
            <p className="text-muted mt-4 md:mt-0 max-w-md text-right">
          ...
          {stats.map((stat, index) => {
            ...
            return (
              <FadeIn key={index} delay={index * 100} className="h-full">
                <div className="p-6 rounded-2xl h-full flex flex-col border border-border bg-surface/50 hover:bg-surface transition-colors group">
                  <div className="flex items-center justify-between mb-8">
                    <div className="rounded-lg bg-accent/10 p-3 text-accent group-hover:text-accent-foreground group-hover:bg-accent transition-colors">
